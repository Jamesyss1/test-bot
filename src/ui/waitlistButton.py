...
class WaitlistForm(ui.Modal):
    def __init__(self):
        ...
        self.region = ui.TextInput(label=f"Gamemodes ({', '.join(listRegions.keys())})", placeholder="Enter your gamemodes", style=nextcord.TextInputStyle.short, required=True)
    ...
    async def callback(self, interaction: nextcord.Interaction):
        try:
            ...
            if role is None: await interaction.response.send_message("Bot not setup correctly, role for gamemodes not found.", ephemeral=True); return
